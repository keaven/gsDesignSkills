...
- `test.type = 4` is standard for confirmatory trials
- Always round to integers with `toInteger()` before reporting
- `gsBoundSummary()` produces gt-ready data frames
- 7 plot types available
- Two-parameter spending function families (Anderson & Clark 2009):
  `sfLogistic`, `sfNormal`, `sfCauchy`, `sfExtremeValue`, `sfBetaDist`
- Sequential p-value theory (Liu & Anderson 2008):
  formal validity proof, spending time vs information fraction
...
design |> gsBoundSummary() |> gt::gt(groupname_col = "Analysis")
```
...
design <- gsSurv(
  ...
  eta = 0.01, gamma = 1, R = 18, minfup = 24,
  sfu = sfLDOF, sfl = sfHSD, sflpar = -2
) |> toInteger()
```
...
**3. Use naturally** --- Claude reads the skill when triggered, applies
code patterns as verified starting points, and consults `llms.txt` for
full API details when needed.
